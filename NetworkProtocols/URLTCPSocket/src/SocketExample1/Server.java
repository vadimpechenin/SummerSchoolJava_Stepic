...
Можно использовать метод setToTimeout(int timeout) класса ServerSocket, чтобы указать время в миллисекундах, на протяжении
которого нужно ожидать подключение клиента. Это позволяет серверу не "зависать", если никто не пытается начать с ним
работаь. Нулевое значение означает бескнечное время ожидания.
